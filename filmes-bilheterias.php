<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Bilheterias</title>

    <link href="css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="css/main.css">
</head>
<body>
    
    	<?php
    		@include "includes/header.php";
    	?>
    <div class="container">
        
        <article class="col-xs-12 col-sm-8 col-md-8 col-lg-8bilheterias">
        	<h1 class="title-news text-justify">Saiba quais são os filmes mais vistos no Brasil e EUA</h1>
            <div class="row">
            <section >
            <img src="images/pessoas-cinema.jpg" alt="pessoas sorrindo no cinema" class="col-xs-12 col-sm-12 col-md-12 col-lg-12 responsive">
            </section>
            </div>
            
            <section class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                <h1><p class="small">Top 10 Bilheterias no Brasil</p></h1><!--<h4>Top 10 Bilheterias nos EUA</h4>-->
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/1.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/007-contra-spectre-60x80.jpg" alt="capa do filme 007" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6" >
                    <p><b>007 Contra Spectre</b></p>
               <p>SONY PICTURES</p>
               <a href="https://www.youtube.com/watch?v=sYp6GpBlb7I" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/2.jpg" alt="">
                </div>
                 <div class="col-md-3 col-sm-4 col-xs-4">      
                <img  src="images/jogos-vorazes-poster-60x80.jpg" alt="Foto Jogos Vorazes" >    
                 </div>
                 <div class="col-md-7 col-sm-6 col-xs-6" >
                    <p><b>Jogos Vorazes: O Final</b></p>
                    <p>PARIS FILMES</p>
                    <a href="https://www.youtube.com/watch?v=0al2icizjqY" target="_blank">Veja O Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/3.jpg" alt="">
                </div>
               <div class="col-md-3 col-sm-4 col-xs-4">     
                <img  src="images/alianca-do-crime-60x80.jpg" alt="Foto do filme a Aliança do Crime" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6" >
                      <p><b>Aliança do Crime</b></p>
                      <p>WARNER BROS</p>
                      <a href="https://www.youtube.com/watch?v=1PnSLtBnh7E" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/4.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/o-reino-gelado-2-60x80.jpg" alt="Foto O Reino Gelado 2" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <p><b>O Reino Gelado 2</b></p>
                    <p>CALIFORNIA FILMES</p>
                    <a href="https://www.youtube.com/watch?v=SDVfzWXcYXk" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/5.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">     
                <img  src="images/no-coracao-do-mar-60x80.jpg" alt="" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">No Coração do Mar</p></b>
                    <p>WARNER BROS</p>
                    <a href="https://www.youtube.com/watch?v=E_hwdx5k5_k" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/6.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">   
                <img  src="images/bem-casados-60x80.jpg" alt="" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">Bem Casados</p></b>
                    <p>IMAGEM FILMES</p>
                    <a href="https://www.youtube.com/watch?v=0wTo79zo11k" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/7.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/victor-frankenstein-60x80.jpg" alt="Foto do Filme Victor Frankenstein" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">Victor Frankenstein</p></b>
                    <p>CENTURY FOX</p>
                    <a href="https://www.youtube.com/watch?v=FCX4UQ19PTs" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/8.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">   
                <img  src="images/o-ultimo-cacador-de-bruxas-60x80.jpg" alt="Foto do filme O Último Caçador de Bruxas" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">O Último Caçador de Bruxas</p></b>
                    <p>UNIVERSAL</p>
                    <a href="https://www.youtube.com/watch?v=ny_RQF0siV4" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/9.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">     
                <img  src="images/quarto-de-guerra-60x80.jpg" alt="Foto do filme Quarto de Guerra" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">Quarto de Guerra</p></b>
                    <p>CANZION FILMES</p>
                    <a href="https://www.youtube.com/watch?v=AIgN1VHP844" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/10.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/o-presente-60x80.jpg" alt="Foto do filme O Presente" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">O Presente</p></b>
                    <p>PLAY ARTE</p>
                    <a href="https://www.youtube.com/watch?v=TqI8lpdK4Cw" target="_blank">Veja o Trailer</a>
                </div>
                </div>

            </section>
         
        <section class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            <h1><p class="small">Top 10 Bilheterias nos EUA</p></h1><!--<h4>Top 10 Bilheterias nos EUA</h4>-->
            
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/1.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">   
                <img  src="images/no-coracao-do-mar-60x80.jpg" alt="" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">No Coração do Mar</p></b>
                    <p>WARNER BROS</p>
                    <a href="https://www.youtube.com/watch?v=E_hwdx5k5_k" target="_blank">Veja o Trailer</a>
                </div>
                </div>
            
                 <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/2.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/jogos-vorazes-poster-60x80.jpg" alt="Foto Jogos Vorazes" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <p><b>Jogos Vorazes: O final</b></p>
                    <p>PARIS FILMES</p>
                    <a href="https://www.youtube.com/watch?v=0al2icizjqY" target="_blank">Veja O Trailer</a>
                </div>
                </div>
            
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/3.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">   
                <img  src="images/007-contra-spectre-60x80.jpg" alt="capa do filme 007" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <p><b>007 Contra Spectre</b></p>
               <p>SONY PICTURES</p>
               <a href="https://www.youtube.com/watch?v=sYp6GpBlb7I" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/4.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">       
                <img  src="images/alianca-do-crime-60x80.jpg" alt="Foto do filme a Aliança do Crime" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                      <p><b>A Aliança do Crime</b></p>
                      <p>WARNER BROS</p>
                      <a href="https://www.youtube.com/watch?v=1PnSLtBnh7E" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/5.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">     
                <img  src="images/o-ultimo-cacador-de-bruxas-60x80.jpg" alt="Foto do filme O Último Caçador de Bruxas" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">O Último Caçador de Bruxas</p></b>
                    <p>UNIVERSAL</p>
                    <a href="https://www.youtube.com/watch?v=ny_RQF0siV4" target="_blank">Veja o Trailer</a>
                </div>
                </div>
            
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/6.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">   
                <img  src="images/no-coracao-do-mar-60x80.jpg" alt="" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">No Coração do Mar</p></b>
                    <p>WARNER BROS</p>
                    <a href="https://www.youtube.com/watch?v=E_hwdx5k5_k" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/7.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">   
                <img  src="images/quarto-de-guerra-60x80.jpg" alt="Foto do filme Quarto de Guerra" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">Quarto de Guerra</p></b>
                    <p>CANZION FILMES</p>
                    <a href="https://www.youtube.com/watch?v=AIgN1VHP844" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/8.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/o-presente-60x80.jpg" alt="Foto do filme O Presente" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">O Presente</p></b>
                    <p>PLAY ARTE</p>
                    <a href="https://www.youtube.com/watch?v=TqI8lpdK4Cw" target="_blank">Veja o Trailer</a>
                </div>
                </div>
                
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/9.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">      
                <img  src="images/victor-frankenstein-60x80.jpg" alt="Foto do Filme Victor Frankenstein" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">Victor Frankenstein</p></b>
                    <p>CENTURY FOX</p>
                    <a href="https://www.youtube.com/watch?v=FCX4UQ19PTs" target="_blank">Veja o Trailer</a>
                </div>
                </div>
            
                <div class="row">
                <div class="col-md-1 col-sm-1 col-xs-1">
                <img src="images/10.jpg" alt="">
                </div>
                <div class="col-md-3 col-sm-4 col-xs-4">    
                <img  src="images/o-natal-dos-coopers-60x80-min.jpg" alt="Foto do O Natal dos Coopers" >    
                </div>      
                <div class="col-md-7 col-sm-6 col-xs-6">
                    <b><p class="negrito">O Natal dos Coopers</p></b>
                    <p>PARIS FILMES</p>
                    <a href="https://www.youtube.com/watch?v=4yhRikngPXQ" target="_blank">Veja o Trailer</a>
                </div>
                </div>
            
                
               
        </section>
        
        </article>
        
        <aside class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="container related-container">
                <h2 class="text-center">Relacionados</h2>
            </div>
            <a href="em-breve.php"><img src="images/tris-convergente.jpg" alt="imagem de teste" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"></a>
            <a href="em-breve.php"><h2 class="news">Assista ao teaser de Convergente da série Divergente</h2></a>
            <a href="filmes-bilheterias.php"> <img src="images/ponte-dos-espioes-grde.jpg" alt="poster do filme Ponte dos Espiões" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"></a>
            <a href="filmes-bilheterias.php"><h2 class="news">Crítica: Ponte dos Espiões surpreende pelo suspense!</h2></a>
            
            <a href="filmes-noticias.php"> <img src="images/self-noticia-gde.jpg" alt="selfie noticias dos atores de Jogos Vorazes" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"></a>
            <a href="filmes-noticias"> <h2 class="news">Elenco da série Jogos Vorazes tiram selfie</h2></a>
        </aside>
        
        <section class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
            <div class="container related-container">
                <h2 class="text-center">Publicidade</h2>
            </div>
            <a href="http://www.cinemark.com.br/portal/cinemark-mania/" target="_blank"><img src="images/cinemark-coca.jpg" alt="imagem propaganda cinemark com coca-cola" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"></a>
            
        </section>
        
    </div>
<?php
    @include "includes/footer.php";
    @include "includes/scripts.php";
?>
</body>
</html>